...
function checkAudioCompatibility() {
    ...
    // Verificación de compatibilidad de audio
...
function updateLyricsDisplay() {
    ...
    // Also enable continue button when audio ends
...
function pauseAudio() {
    console.log('🎵 Función pauseAudio llamada');
    console.log('🎵 Audio player disponible:', !!audioPlayer);
    ...
    if (audioPlayer.paused) {
        ...
        if (playPromise !== undefined) {
            ...
            try {
                ...
                startPreciseAnimation();
                startRomanticEffects();
                console.log('✅ Audio resumed (fallback)');
            } catch (error) {
        ...
        // Pause audio
        audioPlayer.pause();
        document.getElementById('pauseBtn').innerHTML = '<i class="fas fa-play"></i> Play';
...
function showResponseMessage(userSaidYes) {
    ...
    const continueBtn = document.getElementById('continueFromResponse');
    ...
    if (message && responseText) {
        ...
        // Auto-ocultar después de 6 segundos si no se presiona el botón
        setTimeout(() => {
...
function enableContinueButton() {
    console.log('🔧 HABILITANDO BOTÓN CONTINUAR');
    
    // Buscar el botón correcto - puede ser nameSubmit o skipBtn
    const nameSubmit = document.getElementById('nameSubmit');
    const skipBtn = document.getElementById('skipBtn');
    const continueBtn = document.getElementById('continueStoryBtn');
    
    // Habilitar nameSubmit si existe
    if (nameSubmit) {
        nameSubmit.disabled = false;
        nameSubmit.style.opacity = '1';
        nameSubmit.style.cursor = 'pointer';
        nameSubmit.style.pointerEvents = 'auto';
        console.log('✅ Botón nameSubmit habilitado');
    }
    
    // Habilitar skipBtn si existe
    if (skipBtn) {
        skipBtn.disabled = false;
        skipBtn.style.opacity = '1';
        skipBtn.style.cursor = 'pointer';
        skipBtn.style.pointerEvents = 'auto';
        console.log('✅ Botón skipBtn habilitado');
    }
    
    // Habilitar continueBtn si existe
    if (continueBtn) {
        continueBtn.disabled = false;
        continueBtn.style.opacity = '1';
        continueBtn.style.cursor = 'pointer';
        continueBtn.style.pointerEvents = 'auto';
        console.log('✅ Botón continueBtn habilitado');
    }
}
...
function startRomanticEffects() {
    ...
    // Create floating hearts (reduced frequency)
    heartInterval = setInterval(createFloatingHeart, 1500);
    
    // Create sparkles (reduced frequency)
    sparkleInterval = setInterval(createSparkle, 2000);
    
    // Create falling petals (reduced frequency)
    petalInterval = setInterval(createFallingPetal, 3000);
    
    // Create light effects (reduced frequency)
    lightInterval = setInterval(createLightEffect, 4000);
    
    // Create initial burst of effects (reduced quantity)
    setTimeout(() => {
        for (let i = 0; i < 4; i++) {
    ...
    // Create love particles burst (reduced quantity)
    setTimeout(() => {
...
function stopRomanticEffects() {
    ...
    document.querySelectorAll('.romantic-heart, .romantic-sparkle, .romantic-petal, .romantic-light, .love-particle').forEach(el => {
        el.remove();
    });
}
...
function createLoveParticlesBurst() {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        }, i * 100); // Increased delay from 50ms to 100ms
    }
}
...
function showMainStoryImage(imageSrc, duration) {
    ...
    if (storyImage && storyImageElement) {
        // 1. OCULTAR MENSAJE CON TRANSICIÓN
        if (storyMessage) {
            storyMessage.style.transition = 'all 0.5s ease';
        ...
        setTimeout(() => {
            ...
            storyImageElement.onload = function() {
                storyImage.style.display = 'block';
                storyImage.style.opacity = '0';
                storyImage.style.transition = 'opacity 0.5s ease';
                ...
                console.log('✅ IMAGEN PRINCIPAL VISIBLE');
            };
...
function startMainFinalSequence() {
    ...
    if (video && backgroundAudio) {
        ...
        Promise.all([
            ...
            // Audio y video iniciados silenciosamente
...
window.addEventListener('load', function() {
    ...
    if (continueStoryBtn && !continueStoryBtn.hasAttribute('data-listener-added')) {
        continueStoryBtn.setAttribute('data-listener-added', 'true');
